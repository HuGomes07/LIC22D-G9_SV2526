...
begin
	ntClk <= not(Clk);
	Ctrl  : KeyControl port map(Kack => Kack, Kpress => Press, rst => Reset, clk => ntClk, 
		Kval => Kval, Kscan => Scan);
	Scann : KeyScan    port map(Clk => Clk, rst => Reset, Kscan => Scan, KbdLin => KbdLin, 
		KbdCol => KbdCol, K => Kcode, Kpress => Press);
end structural;