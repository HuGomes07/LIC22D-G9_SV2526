...
\fancyhead[LO]{\includegraphics[width=4cm]{01_ISEL-Logotipo-RGB_Horizontal-Compacto-900.png}}
\fancyhead[RO]{
...
O módulo \textit{Keyboard Reader }é constituído por três blocos principais: \textit{i}) o descodificador de teclado (\textit{Key Decode}); \textit{ii}) o bloco de armazenamento (designado por \textit{Ring Buffer}); e \textit{iii}) o bloco de entrega ao consumidor (designado por \textit{Key Transmitter}), de acordo com o diagrama representado na Figura~\ref{fig:keyboardReader}. 
...
\section{Key Decode}
O bloco \textit{Key Decode} implementa um descodificador de um teclado matricial 4x4 por hardware, sendo constituído por três sub-blocos: \textit{i}) um teclado matricial de 4x4; \textit{ii}) o bloco Key Scan, responsável pelo varrimento do teclado; e \textit{iii}) o bloco Key Control, que realiza o controlo do varrimento e o controlo de fluxo, conforme o diagrama de blocos representado na Figura~\ref{fig:keyDecode_diagram}. 

O controlo de fluxo de saída do bloco \textit{Key Decode} (para o módulo \textit{Key Buffer}), define que o sinal $K_{val}$ é ativado quando é detetada a pressão de uma tecla, sendo também disponibilizado o código dessa tecla no barramento $K_{3:0}$. 
...
\section{Ring Buffer}
O bloco \textit{Ring Buffer} implementa uma estrutura de dados para armazenamento de teclas com disciplina FIFO (\textit{First In First Out}), com capacidade de armazenar até dezasseis (16)  palavras de quatro (4) bits.
...
O endereço de escrita/leitura, selecionado por put(get), definido pelo bloco \textit{Memory Address Control }(MAC) composto por dois registos, que contêm o endereço de escrita e leitura, designados por \textit{putIndex} e \textit{getIndex} respetivamente. 
